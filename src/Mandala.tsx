...
const drawPoint = (ctx: CanvasCtx) => (penColor: string) => (
  position: PositionT
) => {
...
const getRotations = (slices: number): DegreeT[] =>
  pipe(
    A.range(0, slices - 1),
...
const normalize = (f: (c: CoordT) => CoordT) => (position: PositionT) =>
  Position.make(f(position.prev))(f(position.current));

const getPoints = (containerSize: Size) => (n: number) => (
  position: PositionT
): PositionT[] => {
  ...
  return (
    <div style={{ display: "flex", flex: 1, flexDirection: "row" }}>
      <div style={{ flex: 1, textAlign: "center", padding: 20 }}>
        <p>
          Click on the box below and start drawing. You can tweak some
          parameters on the right-hand side panel. <br />
          You can also <b>save</b> the image by right-clicking on canvas and
          hitting the "Save image as" option.
        </p>
      </div>
      ...
      <div style={{ flex: 1, flexDirection: "column", padding: 20 }}>
        ...
        <div style={{ flex: 2, flexDirection: "column", marginTop: 30 }}>
          ...
          <input
            type="number"
            value={pens}
            onChange={(e) => setPens(Number.parseInt(e.target.value))}
          />
        </div>
